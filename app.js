...
function handleKeyPress(key) {
  if (winOrLose != "Game Over") {
    if (key.keyCode === 65) {
      dir = DIRECTION.LEFT;
      if (!hittingTheWall() && !checkHorizontalCollision()) {
        deleteTetromino();
        --startX;
        drawTetromino();
      }
...
function checkVerticalCollision() {
  let tetrominoCpy = curTetromino;
  let collision = false;

  for (let i = 0; i < tetrominoCpy.length; ++i) {
    let square = tetrominoCpy[i];
...
function rotateTetromino() {
  ...
  for (let i = 0; i < tetrominoCpy.length; ++i) {
    ...
    let x = tetrominoCpy[i][0];
    let y = tetrominoCpy[i][1];
